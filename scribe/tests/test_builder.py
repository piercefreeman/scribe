...
def test_build_rss_no_draft_notes_in_feed(builder: WebsiteBuilder, tmpdir: str):
    notes = [
        Note(text="PUBLISHED_TEXT", title="PUBLISHED_TITLE", metadata=NoteMetadata(date=datetime.now(), status=NoteStatus.PUBLISHED)),
        Note(text="DRAFT_TEXT", title="DRAFT_TITLE", metadata=NoteMetadata(date=datetime.now(), status=NoteStatus.DRAFT)),
    ]
